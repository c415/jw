...
<body>
				    ...
				    <div class="panel-body">
						<form class="form-horizontal" role="form" action="/admin/editCourse" id="editfrom" method="post">
								  ...
								  <div class="col-sm-10">
									  <select class="form-control"  name="teacherid" id="teacherid">
										  <c:forEach items="${teacherList}" var="item">
											  <option <c:if  test="${item.userid  eq  course.teacherid}"> selected="selected"</c:if> value="${item.userid}">${item.username}</option>
										  </c:forEach>
									  </select>
								  </div>
							  </div>
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
									<select class="form-control" value="${course.coursetype}"  name="coursetype" id="coursetype">
								...
								<div class="col-sm-10">
									<select class="form-control" id="collegeid"  name="collegeid">
										<c:forEach items="${collegeList}" var="item">
											<option <c:if  test="${item.collegeid  eq  course.collegeid}"> selected="selected"</c:if> value="${item.collegeid}">${item.collegename}</option>
										</c:forEach>
									</select>
								</div>
							</div>
	...
	<script type="text/javascript">
        ...
        for (var i=0; i<coursetypeSelect.length; i++) {
            if (coursetypeSelect[i].val() == "${course.coursetype}") {
                coursetypeSelect[i].attr("selected", "selected");
            }
        }
	</script>
</html>